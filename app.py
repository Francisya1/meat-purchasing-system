...
def check_password():
    ...
    with col2:
        with st.form("login_form"):
            ...
            remember_me = False
            if has_cookie_lib: remember_me = st.checkbox("☑️ 記住我 (保持登入狀態 30 天)")
...
STATIC_DICT = {
    ...
    "金錢展": ["金展", "金錢展", "金錢𦟌", "牛展", "shin", "shank", "展"], "肉眼": ["肉眼", "ribeye", "rib-eye"],
    "西冷": ["西冷", "striploin", "sirloin"], "牛柳": ["牛柳", "tenderloin", "fillet"], "豬扒": ["豬扒", "pork chop"],
    "梅肉": ["梅肉", "pork collar", "豬梅肉", "脢肉"], "豬肋排": ["豬肋排", "肋排", "spare rib", "sparerib", "仔骨"],
    "肥牛": ["肥牛", "胸腹", "pastrami", "plate", "short plate", "牛腩"]
}

loading_ph = st.empty(); loading_ph.markdown(get_wavy_loading_html(), unsafe_allow_html=True)
    ...
    for hr in hist_vals[1:]:
        if len(hr) >= 6:
            try:
                ...
                parsed_history.append({'date': datetime.strptime(date_str, "%Y-%m-%d"), 'sku': str(hr[3] if len(hr)>=7 else hr[2]).strip(), 'price': float(hr[5] if len(hr)>=7 else hr[4])})
            except: pass
...
with st.sidebar:
    ...
    if st.button("🚪 登出系統", use_container_width=True):
        ...
        if has_cookie_lib:
            try: cookie_controller.remove('meat_app_auth'); cookie_controller.remove('meat_app_user')
            except: pass
